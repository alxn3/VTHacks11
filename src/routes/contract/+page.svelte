<script lang="ts">
    ...
    function toggleForm(op: "buyForShares" | "sellForShares" | "buyAgainstShares" | "sellAgainstShares") {
        operation = op;
        showForm = true;
    }
...
<div class="flex flex-col gap-8 text-xl font-light">
	...
	<div class="space-y-1">
		...
		<div class="grid grid-cols-[auto,auto,auto,auto,1fr] grid-rows-3 gap-x-3 gap-y-1">
			...
			<Button class="text-sm py-1 px-2 text-green-500" on:click={() => toggleForm("buyForShares")}>Buy</Button>
			<Button class="text-sm py-1 px-2 text-green-500" on:click={() => toggleForm("sellForShares")}>Sell</Button>
			<p class="text-right">8000</p>
			<p>Cope:</p>
			<p class="text-red-500">10 Shares</p>
			<Button class="text-sm py-1 px-2 text-red-500" on:click={() => toggleForm("buyAgainstShares")}>Buy</Button>
			<Button class="text-sm py-1 px-2 text-red-500" on:click={() => toggleForm("sellAgainstShares")}>Sell</Button>
